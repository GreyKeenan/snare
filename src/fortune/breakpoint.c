...
{ //ripped wholesale from commit:6c4d0bd->Orphans_breakpoint_x
	const struct Fortune_point focus1 = (a.y == directix)? b:a;
	const struct Fortune_point focus2 = (a.y == directix)? a:b;
	
	//TODO: Wait, if one of them is on the sweepline, their pararbola is just a straight line.
	//I can simplify in that case, because its just the 'x' of the one on the beachline.
